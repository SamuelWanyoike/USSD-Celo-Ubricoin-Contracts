...
if [ -z "$CONTRACT" ]; then
  yarn run truffle run verify all --network "$NETWORK" --blockscout-url "$BLOCKSCOUT_URL"
else
  yarn run truffle run verify "$CONTRACT" --network "$NETWORK" --blockscout-url "$BLOCKSCOUT_URL"
fi